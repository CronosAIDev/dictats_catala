...
|----|---------|-------------|--------|------------|
| F51 | Dictats en Google Play | [Gameplan](../sections/publicacio/GP_dictats_a_google_play.md) — PWA → TWA → ficha de Play. **Cerradas el 01-09: Fase R (#21, la app verificada por ejecución, Claude responde en producción) y Fase 1 (#17, PWA desplegada en `dictation.generaive.io`)**. Pendientes: F59 (Fase 0 contra MySQL real, espera credenciales), Fase 2 (espera `applicationId`), Fase 3 formularios (espera cuenta de Play), Fase 4 (decisión de nombre) y las pasadas con clave de F55 (#22) | En curso | gerard |
...
| ID | Feature | Descripción | Estado | Prioridad |
|----|---------|-------------|--------|-----------|
| F52 | Identidad propia de Dictats (Fase 0) | Tabla `dictats_usuarios` en `cronosai`, bcrypt, Google OAuth y sesiones en MySQL, calcando `aicamper_app`. Retira la dependencia de `BrandWaiUserProfile` y con ella las contraseñas en texto plano. El progreso se queda en SQLite, indexado por email, así que el histórico no se pierde. Añade el **borrado de cuenta** que Play exige (no estaba en el gameplan). [Issue #16](https://github.com/CronosAIDev/wiki-cronos/issues/16) | **Hecho (v10), sin probar contra MySQL** | Alta |
| F53 | `/.well-known/assetlinks.json` no se servía (Fase 2) | La causa no era `requireAuth` —que ni llega, `express.static` va antes de la sesión— sino que Express ignora por defecto los directorios que empiezan por punto. Montaje propio para `/.well-known` en vez de `dotfiles: 'allow'` en todo `public/`, que abriría cualquier fichero oculto que cayera ahí. Más `scripts/assetlinks.js`, que **se niega a escribir el fichero con una sola huella**: es la trampa que deja la barra de Chrome encima sin dar ningún error | **Hecho (v10)** | Alta |
| F54 | ¿La app funciona hoy? (Fase R) | **Verificado por ejecución el 01-09**: suite 123/123, corrección por texto (3 errores inyectados → 3 detectados), textos personales, perfil y todas las rutas en local; en producción, `claude-opus-4-6` responde con la clave real y el camino de visión también. Quedan como remate manual el dictado por voz y la vista móvil en un Android real (Web Speech vive en el navegador). [Issue #21](https://github.com/CronosAIDev/wiki-cronos/issues/21), cerrada | **Hecho** | Alta |
| F55 | Banco de pruebas de modelos y coste por corrección (Fase R) | Errores inyectados por clase (acento abierto/cerrado, ela geminada, apostrofació, dièresi, pronoms febles, b/v) sobre los textos del banco: el ground truth sale gratis porque el texto correcto ya lo tenemos. Mide recall, falsos positivos, acierto de clasificación, JSON válido, latencia y **coste real** del campo `usage`. Los falsos positivos pesan más: enseñan ortografía equivocada. La elección de modelo es de Óscar. Ya no hace falta tocar código para probar: `DICTATS_MODEL` lo escoge (v6). ⚠️ **La #22 se escribió para la arquitectura de antes**: desde F31 el modelo no busca los errores, así que recall, falsos positivos y clasificación del camino de texto son deterministas y no dependen del modelo. Donde la elección sí decide es en la **foto**, porque el modelo transcribe y una transcripción falsa se le enseña al usuario como falta suya. Arnés hecho y verificado en `scripts/benchmark/` (recall 100% en las 8 clases); faltan las pasadas con clave de API | **Parcial** | Alta |
| F57 | Dos apóstrofos seguidos se desmontan | `L'oli d'oliva` escrito `El oli de oliva`: los errores se encuentran todos, pero se reportan dos de más y el texto que se enseña sale desplazado. Sale del banco de pruebas (F55). Es el mismo mal que el caso ya arreglado, pero con dos apóstrofos consecutivos, y arreglarlo pide rehacer `ajuntaApostrofs()` para que trate secuencias y no parejas | Pendiente | Media |
| F58 | El banco no cubre las clases que más discriminan | Palabras tocables en los 30 textos: ela geminada **5**, dièresi **7**, b/v **7** — frente a 140 de acento general. Son justo las que separan un modelo que sabe catalán de uno que arrastra el castellano. Hace falta un texto escrito a propósito que las cargue, para que el banco sirva de verdad al comparar modelos en el camino de la foto | Pendiente | Media |
| F59 | Probar la identidad propia contra MySQL de verdad | El código de F52 tiene 34 comprobaciones con una base falsa, pero **nunca ha hablado con `cronosai`**: falta crear el usuario de base de datos, correr `scripts/db/001_usuaris.sql`, dar de alta una cuenta real y completar una vuelta entera de Google OAuth con credenciales del GCP `kairos-family-app`. Hasta entonces la Fase 0 no se puede dar por cerrada | Pendiente | Alta |
| F60 | Migrar las cuentas que ya existen | Producción tiene 2 usuarios en `BrandWaiUserProfile`. Con identidad propia dejan de poder entrar hasta que se den de alta. Su progreso **no se pierde** (la clave es el email), pero hay que decidir si se les avisa, se les crea la cuenta o se deja que se registren | Pendiente | Media |
| F61 | Nombre público, `applicationId` y ficha de Play (Fase 4) | Material en [`docs/sections/publicacio/FITXA_PLAY.md`](../sections/publicacio/FITXA_PLAY.md): nombre, `applicationId` con sus alternativas, keywords separando lo comprobado de lo que es hipótesis, descripción corta y larga, y el guion de capturas. **El dato que lo cambia todo**: el C2 es requisito docente desde el curso 25-26 y solo lo tiene el 25% del profesorado — unas 70.000 personas, que son el público exacto. **Y su trampa**: el examen de C2 no es un dictado (el Área 2 es de opción múltiple), así que prometer preparación sería el mismo error que el gameplan avisa para la sintaxis. Falta que Óscar decida nombre, `applicationId` y si la ficha menciona el C2 | **Redactado, pendiente de decisión** | Alta |
| F62 | Gráfico de cabecera 1024×500 para Play | No existe. El icono de 512 sí (`public/icons/icon-512.png`). Las capturas dependen del despliegue y de la clave de API | Pendiente | Media |
| F63 | Política de privacidad, Data Safety y clasificación (Fase 3) | Página `/privacitat` servida por la app **sin sesión**, escrita auditando el código línea a línea. Respuestas del formulario de Data Safety y del cuestionario IARC en [`docs/sections/publicacio/DATA_SAFETY.md`](../sections/publicacio/DATA_SAFETY.md). Dato que importa: al camino de texto **solo salen las palabras falladas**, no el texto entero — consecuencia de F31. Y la foto no se guarda en ningún sitio (`memoryStorage`) | **Hecho, pendiente de publicar** | Alta |
| F64 | Botón para reportar contenido generado con IA | Play trata las apps que generan contenido con IA como área regulada y exige **reportar contenido ofensivo desde dentro de la app**. Las explicaciones de cada error las escribe el modelo, así que Dictats entra. Botón ⚑ en cada explicación generada y en el mensaje final, `POST /api/report`, tabla `content_reports`. Solo aparece sobre texto que ha escrito el modelo, y no oculta nada automáticamente. No estaba en la Issue #19 | **Hecho (v10)** | Alta |
| F65 | Reclutar los 20 testers docentes | Son **14 días de calendario** que no han empezado. Antes hay que decidir si la cuenta de desarrollador es personal (requiere los testers) o de organización (no, pero pide D-U-N-S y también tarda). Decisión de Óscar | Pendiente | Alta |
| F56 | Límites de llamadas en las rutas que cuestan dinero | Dos capas por ruta —ráfaga por hora y techo diario—, más justas en la foto porque viaja la imagen entera. **Se cuenta por persona, no por IP**: el público son docentes y una sala de profesores sale por una sola IP; contar por IP haría que uno se comiera el bloqueo de un compañero. En `/correct-image` el límite va **antes** de multer, así que un envío de 9 MB se corta en 2 ms. Concreta F14. Falta el techo de gasto global, que es parte de #22 | **Hecho (v10)** | Media |
